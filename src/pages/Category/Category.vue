...
export default {
  ...
  methods: {
    ...
    async _initData () {
        ...
        // homeApi/categories?Geek-James=99189758328578919283
        let url = 'homeApi/categories?Geek-James=' + this.randomCode(20);
        let _this = this;
        this.$fetchGet(url).then(res=>{
            if(res.data.success){
                _this.categoriesData = res.data.data.cate;
            }
        })

        // homeApi/categoriesdetail/lk001?Geek-James=99189758328578919283
        let urlR = 'homeApi/categoriesdetail/lk001?Geek-James=' + this.randomCode(20);
        this.$fetchGet(urlR).then(res=>{
            if(res.data.success){
                _this.categoriesDetailData = res.data.data.cate;
            }
        })

      // 1.3. 隐藏loading框
      this.isShowLoading = false
      console.log('textContent1',this.$el.textContent) // => 'not updated'
      // 1.4.初始化滚动视图
      this.$nextTick(() => {
        console.log('textContent2',this.$el.textContent) // => 'updated'
        if (!this.leftScroll) {
    ...
    //   }
      ...
      let urlR = 'homeApi/categoriesdetail'+param+'?Geek-James=' + this.randomCode(20);
      let _this = this;
      await this.$fetchGet(urlR).then(res=>{
            if(res.data.success){
                _this.categoriesDetailData = res.data.data.cate;
            }
        })
