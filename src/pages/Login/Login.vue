...
export default {
    ...
    methods:{
        ...
        async sendVerifyCode () {
            ...
            // 4.2 获取短信验证码
            //send_code?phoneNumber=13715017405&Geek-James=77917386926488579547
            let url = 'send_code?phoneNumber='+this.login_phone+'&Geek-James='+ this.randomCode(20);
            let _this = this
            await this.$fetchGet(url).then(result=>{
                console.log(result)
                if (result.data.success_code == 200) {
                    _this.smsCaptchaResult = result.data.data.code;
                    // 4.3  获取验证码成功
                    Dialog.alert({
                        title: _this.$t('login.tipTile'),
                        message: _this.$t('login.message') + result.data.data.code
                    }).then(() => {
                    });
                }
            })
        ...
        async login () {
            if (this.isShowSMSLogin) {
                ...
                // login_code post请求
                let url='login_code'
                await this.$fetchPost(url,{captcha: _this.smsCaptchaResult,phone:_this.login_phone}).then(res=>{
                    console.log(res)
                    // 设置userInfo 保存到vuex和本地
                    _this.syncuserInfo(res.data.data);
                    _this.$router.back();
                })
            } else {
                // 5.2 账号密码登录
        ...
        async register () {
            if (this.register_userName.length < 1) {
                ...
                // login_code post请求
                let url='login_code'
                await this.$fetchPost(url,{captcha: _this.register_pwd,phone:_this.login_phone}).then(res=>{
                    // 设置userInfo 保存到vuex和本地
                    _this.syncuserInfo(res.data.data);
                    _this.$router.back();
                })
